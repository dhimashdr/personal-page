...
export default function SkillCards({skills} : Skills){
    return <div className={`grid grid-cols-1 bg-linear-to-b from-black to-sky-950 border-gray-600 ${skills.diff === "Intermediate" ? "md:border-x" : ""}`}>
        ...
        <div className="flex gap-4 mx-auto py-5 lg:py-10">
            {skills.skill.map((item) => {
                return <div key={item} className="flex items-center-safe gap-2 bg-gray-300/10 pr-1">
                    <Icon 

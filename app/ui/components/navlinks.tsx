...
export default function NavLinks(){
    ...
    return <div className="mx-auto w-full py-4 px-8 md:px-28 lg:w-16 grid grid-cols-5 lg:grid-cols-1 fixed lg:gap-6 bottom-0 lg:top-0 lg:p-0 lg:py-40 z-50 bg-black/70 backdrop-blur-md lg:bg-black">
        {navs.map(nav => {
            ...
            return <MLink initial={{scale: 1}} whileTap={{scale: 0.9}} transition={{ease: 'easeOut', duration: 0.1}} href={nav.url} key={nav.title} className={`${pathname.includes(nav.url) && (nav.url !== "/" || pathname === "/") ? 'bg-blue-500/30' : 'bg-none'} px-2 transition-all duration-200 hover:bg-blue-500/30 ease-in rounded-sm my-auto mx-auto`}>
